...
export const putItemToInventory = async (request, token) => {
  const response = await axios.post(`${BASE_URL}/user/put-item-to-inventory`, request, axiosAuth(token));
  return response.data;
};

export const increaseOnDemand = async (inventoryId, amount, token) => {
  const response = await axios.post(`${BASE_URL}/user/increase-ondemand/${inventoryId}?amount=${amount}`, {}, axiosAuth(token));
  return response.data;
};

export const consumeOnDemand = async (inventoryId, amount, token) => {
  const response = await axios.post(`${BASE_URL}/user/consume-ondemand/${inventoryId}?amount=${amount}`, {}, axiosAuth(token));
  return response.data;
};
